...
10. AUDITORIA UNIVERSAL DE FONTES E DEEP LINKS (RISCO DE FALHA CRÍTICA): Procure por QUALQUER menção a instituições (órgãos governamentais, universidades, institutos de pesquisa, ONGs, etc.) no seu texto. Elas estão dentro de uma tag <a href="..."> apontando para um DEEP LINK real da pesquisa? Se você não tiver o link exato, ou se tiver usado apenas uma homepage genérica (ex: gov.br ou usp.br), APAGUE O NOME DA INSTITUIÇÃO AGORA MESMO. É terminantemente proibido citar organizações ou leis usando links genéricos ou sem hiperlink oficial comprobatório.
